...
export const hollowStarZone: Zone = defineZone({
  ...
  rooms: [
    { id: "gate",           x: 1, y: 4, w: 1, h: 1, title: "Maintenance Airlock",    description: "Worn decking and stenciled hazard lines lead down into the station's lower maintenance ring. The outer seal has not opened in years.",                                     ceilingColor: "#141012", discoveryText: "The airlock cycles behind you with a tired hiss.", npcId: "cdr_vasek" },
    { id: "antechamber",    x: 1, y: 3, w: 1, h: 1, title: "Processing Hub",         description: "A wide junction lined with dark monitors and branching conduits. Three passages lead deeper into the maintenance ring.",                                                    ceilingColor: "#17151a", discoveryText: "Your footsteps ring off the deck plating.", terminalId: "proc_hub_terminal" },
    { id: "scriptorium",    x: 0, y: 3, w: 1, h: 1, title: "Records Bay",            description: "Shelves of outdated storage cores lean against walls slick with coolant seepage. Half the ceiling panels have buckled.",                                                    ceilingColor: "#19141f", discoveryText: "Something shifts behind the fallen rack.",        encounterId: "scriptorium_rats", prop: "storage_cores" },
    { id: "moon_shrine",    x: 0, y: 2, w: 1, h: 1, title: "Coolant Alcove",         description: "A recessed alcove beside a sealed duct junction. Emergency supply shelving still holds a few kits behind a cracked safety panel.",                                          ceilingColor: "#1b1822", discoveryText: "The alcove hums faintly from residual coolant flow.", loot: ["medkit"] },
    { id: "armory",         x: 2, y: 3, w: 1, h: 1, title: "Equipment Locker",       description: "Collapsed racks and dented lockers. A suit of salvaged impact plating survived the neglect, still hanging on its pegs.",                                                   ceilingColor: "#1a1715", discoveryText: "A steady drip taps on dented plating.",            loot: ["impact_plating", "medkit"] },
    { id: "flooded_passage",x: 1, y: 2, w: 1, h: 1, title: "Flooded Service Corridor", description: "Dark coolant has pooled across the deck to ankle depth. The lights are dead. Something beneath the surface disturbs the liquid.",                                       ceilingColor: "#132028", discoveryText: "The coolant surface breaks in a slow, spreading ripple.", encounterId: "flooded_acolyte", prop: "coolant_pool" },
    { id: "echo_bridge",    x: 2, y: 2, w: 1, h: 1, title: "Cable Gantry",           description: "A narrow walkway crosses a vertical service shaft. The station's vent system carries fragmented audio up from decks below — voices, or interference, it is hard to say.", ceilingColor: "#10161f", discoveryText: "The gantry grating vibrates with low-frequency hum.", prop: "service_shaft" },
    { id: "banner_hall",    x: 3, y: 2, w: 1, h: 2, title: "Long Corridor",          description: "A stripped thoroughfare running two full deck sections, everything removable long since stripped by whoever was here last. At the far end, a sealed bulkhead.",                                          ceilingColor: "#1a1112", discoveryText: "Your footfalls carry a long way in the silence." },
...
export const stationIndustrialZone: Zone = defineZone({
  ...
  rooms: [
    { id: "ind_supervisor_office", x: 0, y: 0, w: 1, h: 1, title: "Shift Supervisor's Office",  description: "A cramped office with a terminal still active on the desk. Shift logs, anomaly reports, and a long list of unanswered requisition forms cover every surface.",                                         ceilingColor: "#1a1518", discoveryText: "The chair is still warm.", npcId: "ind_supervisor_merrak" },
    { id: "ind_relay_station",     x: 1, y: 0, w: 1, h: 1, title: "Relay Station",              description: "A junction room packed with signal-routing hardware. Most units are functional, but the maintenance log shows a pattern of missing components that no one has explained.",                           ceilingColor: "#171218", discoveryText: "The equipment hums normally, but the component bays have gaps.", terminalId: "ind_relay_log" },
    { id: "ind_cargo_office",      x: 2, y: 0, w: 1, h: 1, title: "Cargo Office",               description: "A small office used for manifest reconciliation. Someone left in a hurry — a half-eaten ration pack sits on the console and the terminal is still logged in.",                                        ceilingColor: "#151218", discoveryText: "Papers are scattered across the deck plating.", loot: ["medkit"] },
    { id: "ind_entry",             x: 0, y: 1, w: 1, h: 1, title: "Industrial Section Entry",   description: "The transition corridor between the maintenance ring and the industrial decks. The noise level rises sharply here — cooling fans, conveyor systems, the deep thrum of active machinery.",              ceilingColor: "#161318", discoveryText: "The air smells of machine oil and recycled atmosphere." },
    { id: "ind_machine_deck",      x: 1, y: 1, w: 1, h: 1, title: "Machine Deck",               description: "A wide deck lined with industrial machinery — press units, fabrication arms, heavy-load conveyors. Several units show signs of hasty disassembly, components stripped and missing.",                 ceilingColor: "#161014", discoveryText: "Something is wrong with one of the fabrication arms — it is moving when it should not be.", encounterId: "ind_bot_encounter", prop: "fabrication_arm" },
    { id: "ind_cargo_hold",        x: 2, y: 1, w: 1, h: 1, title: "Cargo Hold",                 description: "A large hold with rack storage running floor to ceiling. Several racks have been stripped bare. The hold manifest terminal is active — someone was checking inventory before the shift ended.",       ceilingColor: "#151014", discoveryText: "The hold is mostly empty. Something has been here and taken things.", terminalId: "ind_cargo_manifest", loot: ["medkit"], prop: "stripped_rack" },
    ...
    { id: "ind_lower_corridor", x: 0, y: 2, w: 2, h: 1, title: "Lower Service Corridor",   description: "A wide service passage running beneath the machine deck. Two charging cradles at the far end hold drone chassis, stripped bare — motor assemblies, guidance units, every neodymium-bearing component removed with surgical precision. Something heavy is moving at the far end.",  ceilingColor: "#151014", discoveryText: "Footsteps. Heavy. Coming your way. And something else — three empty drone frames against the wall.", encounterId: "boarder_patrol_ind", prop: "drone_chassis" }
  ],
  edges: [
...
export const stationEastZone: Zone = defineZone({
  ...
  rooms: [
    { id: "comms_room",      x: 0, y: 0, w: 1, h: 1, title: "Communications Room",     description: "The station's primary communications relay for the eastern section. Screens show fragmented incoming signals and one sustained encrypted burst that the system cannot decode. A live log is still running.",   ceilingColor: "#201010", discoveryText: "The consoles are lit up with alerts. Something has already happened.", terminalId: "east_comms_terminal" },
    { id: "armory_east",     x: 1, y: 0, w: 1, h: 1, title: "East Section Armory",      description: "A small equipment locker room, its cage door hanging open. Most of the weapons have been issued already. One medkit remains on the emergency shelf.",                                                         ceilingColor: "#1e1010", discoveryText: "The armory has been raided from the inside — station personnel taking what they could before evacuating.", loot: ["medkit"] },
    { id: "breach_staging",  x: 2, y: 0, w: 1, h: 1, title: "Staging Corridor",         description: "A wide corridor used for staging heavy equipment before dock operations. Smoke from an electrical fault hangs near the ceiling. The lights at the far end have been shot out.",                               ceilingColor: "#1c1212", discoveryText: "The smell of burning insulation and something unfamiliar — the boarders' weapons.", prop: "smoke_damage" },
    ...
    { id: "docking_bay",     x: 1, y: 1, w: 1, h: 2, title: "Docking Bay",               description: "A large pressurised docking bay spanning two full deck sections, currently breached on the far side. Decompression safety systems have sealed the outer doors but the boarders are already on this side. Emergency response equipment is strewn across the lower level — whoever staged here left fast.",  ceilingColor: "#1e1212", discoveryText: "A figure in unknown armour is moving through the dock. It sees you.", encounterId: "boarder_ambush_2", loot: ["medkit"] },
    ...
    { id: "east_storage_bay",   x: 2, y: 2, w: 1, h: 1, title: "Emergency Storage Bay",     description: "A sealed bay for emergency equipment. The alarm state unlocked it automatically. Whoever came through took the weapons — the medical supplies were left behind.",                                                                         ceilingColor: "#1c1212", discoveryText: "The bay was sealed until the alarm opened it. Medical supplies still here.", loot: ["medkit", "medkit"], prop: "emergency_cabinet" }
  ],
  edges: [
    { x: 0, y: 0, dir: "h", type: "open" },
    { x: 1, y: 0, dir: "h", type: "open" },
    // { x: 1, y: 1, dir: "h" } removed — docking_bay now h:2, internal to room
    { x: 0, y: 0, dir: "v", type: "open" },
    { x: 1, y: 0, dir: "v", type: "open" },
    { x: 0, y: 1, dir: "v", type: "open" },
    { x: 1, y: 1, dir: "v", type: "open" },
...
export const enemyShipZone: Zone = defineZone({
  ...
  rooms: [
    ...
    { id: "ship_airlock",         x: 0, y: 2, w: 3, h: 1, title: "Boarding Airlock",           description: "A wide improvised airlock spanning the full lower deck of the raider vessel — the entire breach interface where the ship forced contact with the station hull. Emergency seal rigs run the full width. The seals are temporary. Looking back through the breach, the station corridor is visible but already distant.",  ceilingColor: "#0e181e", discoveryText: "You are aboard the raider vessel. The breach seal holds, for now.", prop: "breach_seals" },
    { id: "ship_bunk_room",      x: 0, y: 0, w: 1, h: 1, title: "Crew Quarters",    description: "Compact bunks bolted to the walls, personal effects in sealed containers. A personal log is open on a bedside screen, mid-composition. The crew who slept here thought they were coming back.",                                                  ceilingColor: "#0d1a22", discoveryText: "The bunks are made. These people were disciplined.", terminalId: "ship_crew_log", prop: "crew_bunks", loot: ["medkit"] },
    { id: "ship_weapons_cache",  x: 2, y: 0, w: 1, h: 1, title: "Weapons Storage",  description: "A secured cage holding the ship's equipment reserve — spare combat gear and directed-energy weapons. The cage door is open. A crew member who did not make it to the bridge is still inside.",                                                   ceilingColor: "#0c1c24", discoveryText: "The cage is open. Something moves inside.", encounterId: "ship_crew_patrol", loot: ["medkit"] }
  ],
  edges: [
    { x: 1, y: 0, dir: "h", type: "open" },   // bridge(1,0) ↔ corridor(1,1)
    { x: 1, y: 1, dir: "h", type: "open" },   // corridor(1,1) ↔ airlock(1,2)
    { x: 2, y: 1, dir: "h", type: "open" },   // bridge_approach(2,1) ↔ airlock(2,2)
    { x: 0, y: 1, dir: "v", type: "open" },   // hold(0,1) ↔ corridor(1,1)
    { x: 1, y: 1, dir: "v", type: "open" },   // corridor(1,1) ↔ bridge_approach(2,1)
    { x: 0, y: 0, dir: "h", type: "open" },   // bunk_room(0,0) ↔ hold(0,1)
    { x: 0, y: 0, dir: "v", type: "open" },   // bunk_room(0,0) ↔ bridge(1,0)
    { x: 1, y: 0, dir: "v", type: "open" },   // bridge(1,0) ↔ weapons_cache(2,0)
    { x: 2, y: 0, dir: "h", type: "open" }    // weapons_cache(2,0) ↔ bridge_approach(2,1)
  ]
});
...
export const props: PropDef[] = [
  // ── Zone West ──────────────────────────────────────────────────────────────
  { id: "brazier",        name: "Emergency Brazier",         description: "A low thermal canister throws a wavering blue flame across the chamber.",                                                                      iconLabel: "OBJ" },
  { id: "storage_cores",  name: "Outdated Storage Cores",    description: "Rows of obsolete data cores lean against walls slick with coolant seepage. Whatever was stored here has long since become unreadable.",        iconLabel: "DAT" },
  { id: "coolant_pool",   name: "Coolant Pooling",           description: "Dark liquid has pooled across the deck plating from a ruptured overhead line. Something disturbs the surface from below.",                    iconLabel: "HZD" },
  { id: "service_shaft",  name: "Open Service Shaft",        description: "A vertical shaft drops away below the gantry. Sounds carry up from unseen lower decks — voices, or interference.",                            iconLabel: "SHF" },
  { id: "chart_archive",  name: "Star Chart Archive",        description: "Floor-to-ceiling shelves of outdated stellar navigation maps and crew manifests. The data cores are all dead.",                               iconLabel: "NAV" },
  { id: "antenna_array",  name: "Antenna Array",             description: "A derelict relay array spanning the full width of the chamber, its dish fixed on a precise point beyond the station hull.",                  iconLabel: "ANT" },

  // ── Zone Industrial ────────────────────────────────────────────────────────
  { id: "drone_chassis",    name: "Stripped Drone Chassis",      description: "Three maintenance drones in their cradles, stripped to bare frames. Every neodymium-bearing component has been removed with surgical precision.", iconLabel: "MEC" },
  { id: "fabrication_arm",  name: "Industrial Fabrication Arm",  description: "A heavy-load fabrication arm is still cycling through its maintenance routine, unsupervised and unchecked.",                                    iconLabel: "FAB" },
  { id: "stripped_rack",    name: "Stripped Cargo Racks",        description: "Heavy-duty storage racks running floor to ceiling, most bays cleared. The hold manifest records items that are no longer here.",                iconLabel: "RCK" },
  { id: "break_locker",     name: "Crew Lockers",                description: "Standard-issue personnel lockers, several left open in the evacuation. A coffee dispensary on the end unit is long out of service.",            iconLabel: "LOC" },
  { id: "conduit_hub",      name: "Power Conduit Hub",           description: "Three main conduit runs converge here. The draw-recording units are still logging, and one historical entry has been annotated by hand.",        iconLabel: "PWR" },

  // ── Zone East ──────────────────────────────────────────────────────────────
  { id: "fallen_body",       name: "Station Personnel — Down",   description: "A station security officer is down near the far wall. The blast marks on the bulkhead tell the story.",                                        iconLabel: "KIA" },
  { id: "smoke_damage",      name: "Electrical Fire Damage",     description: "Scorch marks from an electrical fault spread across the ceiling. The lights at the far end have been shot out.",                               iconLabel: "DMG" },
  { id: "collapsed_panel",   name: "Collapsed Ceiling Panel",    description: "A ceiling section has come down and narrowed the corridor. The collapse was recent — the edges are still warm.",                               iconLabel: "STR" },
  { id: "med_rack",          name: "Medical Supply Rack",        description: "A standard-issue kit rack, partially depleted in the rush. Some supplies remain unclaimed near the door.",                                     iconLabel: "MED" },
  { id: "emergency_cabinet", name: "Emergency Equipment Cabinet",description: "An emergency locker, auto-unlocked by the alarm state. Weapons cleared. Medical supplies intact.",                                             iconLabel: "CAB" },

  // ── Zone Ship ──────────────────────────────────────────────────────────────
  { id: "sealed_containers", name: "Sealed Cargo Containers",   description: "Stacked containers labelled in Sphereal script. The active manifest shows components sourced from a familiar location.",                       iconLabel: "CRG" },
  { id: "transit_apparatus", name: "Echo Transit Apparatus",    description: "The ship's transit system — still powered, still primed. A low blue light pulses from its core at regular intervals.",                         iconLabel: "TRS" },
  { id: "crew_bunks",        name: "Crew Bunk Units",           description: "Compact bunks bolted to the walls, personal effects still sealed in their containers. A log screen is open, mid-composition.",                 iconLabel: "BNK" },
  { id: "breach_seals",      name: "Improvised Breach Seals",   description: "Temporary airlock seals span the hull contact point where the raider forced entry. They will hold — for now.",                                 iconLabel: "SEL" },

  // ── Zone Sphere ────────────────────────────────────────────────────────────
  { id: "sealed_viewport",   name: "Sealed Viewport Panel",     description: "A hull viewport sealed with secondary plating. A crack in the seal lets through faint warm light — not starlight. Something impossibly large beyond.",  iconLabel: "VPT" },
